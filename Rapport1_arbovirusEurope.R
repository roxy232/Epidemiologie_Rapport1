#--------------NICHE ECOLOGIQUE d'un arbovirus en Europe------------------------
...
par(mfrow=c(4,5), oma=c(0,0,1.5,0), mar=c(0.75,0,0,0), lwd=0.2,  col="gray30", col.axis="gray30", fg="gray30") #on figuration de la fenêtre graphique
...
mtext("Température été", side=3, line=1, cex=0.65, col="gray30") 
...
plot(EU, add=T, border="gray50", lwd=0.5) 
mtext("Température Hiver", side=3, line=1, cex=0.65, col="gray30") 
mtext("(°C)", side=3, line=0.5, cex=0.65, col="gray30")
...
#fORêTs (les primaires sont les plus vielles forêts n'ayant pas été affectées par l'activité humaine, l'échelle est le pourcentage de forêt 1er/pixel)
...
records_Arbovirus = read.csv("C:/Users/hoste/OneDrive - Université Libre de Bruxelles/Documents/Cours/VIRUS/Tdegrp/Binome_06/Arbovirus_risk_mapping_simulated_dataset_2-2.CSV", head=T)[,c("longitude","latitude")] #fichier de localisation des présences détectées du virus
...
background = template #part de raster
...
number_of_replicates = 50 # nombre de répétition que le modèle va générer pour stabiliser les résultats statistique 
dataframes = list() 
...
for (i in 1:number_of_replicates) {
  ...
  data = cbind(data, raster::extract(stack(envVariables), data[,c(2:3)])) #compléter avec les valeurs environnementales 
...
#c'est à dire si deux points sont trop proches ou le modèle va plus lié le point à l'environnement mais le mettre au hasard

#Eviter les surajustement (s'adapte)
par(oma=c(0,0,0,0), mar=c(2,2.2,0,0))
#Corrélation spatiale sur chaques réplicats (les 50) 
for (i in 1:number_of_replicates) {
  data = dataframes[[i]]
...
#A partir de 2100, on évite l'autocorrélation (au vue du graphique)
...
theRanges=c(2100,2100)*1000 #distance spatiale (en dessous de 0 du grahique d'avant)
gbm.x= envVariableNames 
gbm.y="response" #1 VS 0
offset=NULL;tree.complexity=5 #nombre de noeud par arbres => les intéractions possibles entre variables 
learning.rate=0.005 # assez conservatif 

bag.fraction=0.80 #80% données utilisées pour entrainement
site.weights=rep(1,dim(data)[1]) #on a le même poids 
...
step.size=5 #déviance prédictif (sert à la correction entre les arbres)
max.trees=10000 
#Arrêt
...
library(sf)
sf::sf_use_s2(FALSE) #bloquer les résultats géomatriques spatiales pour éviter des bugs pour calculer les ditances
brt_models=list()
predictions = list()
for(i in 1:number_of_replicates){
  data=dataframes[[i]]
  ...
  myblocks=cv_spatial(spdf,column="response",k=n.folds,size=theRanges[1],selection="random") #cross-validation
  fold.vector=myblocks$folds_ids 
  n.trees=10 #BRT entrainement
  ...
  dataframe=as.data.frame(stack(envVariables)) #projection spatiale(création d'une carte de risque)
  dataframe=dataframe[which(!is.na(rowMeans(dataframe))),] 
...
write.csv(AUCs,"AUC_value_replicates.csv",row.names=F,quote=F) #voir tt les résulats
...
mean_risk_map = mean(my_stack)#la moyenne des prédictions faites 
...
for (i in 1:19) { 
  ...
  plot(EU, add=T, border="gray50", lwd=0.5) 
  mtext("Ecological suitability", side=3, line=0.3, cex=0.65, col="gray30") # (1° part)
...
cols_standard = rev(colorRampPalette(brewer.pal(9,"RdYlBu"))(100))
sd_risk_map = calc(my_stack, fun=sd) #l'écart-type (Standard Deviation)es 50 réplicats
cv_risk_map = sd_risk_map / (mean_risk_map + 0.0001) #calcule le Coefficient de Variation (SD / Moyenne) + 0.0001 pour éviter les erreur
valeurs_cv = getValues(cv_risk_map) #CALCUL DU MAXIMUM COHÉRENT 
...
plot(cv_map_final, 
     ...
     axes=F, 
     box=F) # L'échelle s'arrête exactement au max cohérent
...
plot(cv_map_final, col=cols_standard, legend.only=T, add=T, 
     legend.width=1, legend.shrink=0.5, 
     axis.args=list(at=seq(0, 1, 0.2), labels=seq(0, 1, 0.2), cex.axis=0.8))


#Etape 7:  prédiction performance de le niche écologique :
...
for(i in 1:length(brt_models)) {
  ...
  P= sum(responses==1) #présence
  A= sum(responses==0) #pseudo-absence
  prev=P/(P+A)
  x=(P/A)*((1-prev)/prev);SI_ppc= 0 #vérifie les zones à risque de fausses données
  for (threshold in seq(0,1,0.01)) { #boucle de seuil trouver l'optimum
...
#MeanSIppc=0.96,SIppcrange=[0.93,0.98] => c'est robuste 
...
# Etape 8 : importances relatives avec moyenne, écart-type, IC 95% et IC 5%
RI_summary = data.frame(row.names = envVariableNames)

for(i in 1:dim(relative_importances)[2]) {
  RI_summary[i, "mean_RI"] = round(mean(relative_importances[,i]), 1)
  RI_summary[i, "sd_RI"]   = round(sd(relative_importances[,i]), 1)
  ci95 = round(t.test(relative_importances[,i], conf.level=0.95)$conf.int[1:2], 1)
...
#STEP9 : Visualisation de la niche écologique du virus : 
brt_models=readRDS("BRT_model_replicates.rds")
